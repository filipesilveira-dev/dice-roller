...
- **Middleware `persist` do Zustand:** recurso fornecido pela ferramente de controle de estado global Zustand que possibilita a persistência de dados. Ele é utilizado na criação do store (estados e ações) e por meio do `partialize` podemos especificar qual dos dados será persistido. No projeto, foi utilizado em `useDiceStore` para persistir "dices" no localStorage. Ou seja, é armazenado localmente um array de objetos contendo cada dado em tela com o respectido "id" e número de "faces". Uma funcionalidade futura armazenará o valor de cada dado, possibilitando a soma total.
